...
def chunk_hierarchical(text: str, parent_size: int = HIERARCHICAL_PARENT_SIZE,
    ...
    for p in parents:
        ...
        for sent in p_sentences:
            if len(current_child_text) + len(sent) > child_size and current_child_text:
    ...
    for part in sections:
        ...
        if re.match(r'^#{1,3}\s+', part):
            if current_content:
                ...
                chunks.append(Chunk(text=chunk_text, metadata={**metadata, "section": current_header, "strategy": "structure"}))
            current_header = part
            current_content = ""
        else:
